...
void settings_restore(uint8_t restore_flag) {  
  if (restore_flag & SETTINGS_RESTORE_DEFAULTS) {
	...
	laserPowerSettings = DEFAULT_LASER_POWER;	// JDU : 20161113
...
uint8_t read_global_settings() {
  ...
  // par defaut, laser power à 100%
  laserPowerSettings = DEFAULT_LASER_POWER;
  return(true);
}


// A helper method to set settings from command line
uint8_t settings_store_global_setting(uint8_t parameter, float value, uint8_t save) {
  ...
  if (save == 1) {	// if 1st char is %, not save in eeprom - JDU 20161116
	write_global_settings();
  }
  return(STATUS_OK);
}
